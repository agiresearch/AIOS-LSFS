...
class RetrieveSummary(BaseAgent):
    ...
    def match(self, input, mode):
        if mode == "add":
            ...
            match = re.search(pattern, input)
            if match:
                query, metaname, sub_name, method = (
                    match.groups(1),
                    match.groups(2),
                    match.groups(3),
                    match.groups(4),
            ...
            match = re.search(pattern, input)
            if match:
                metaname2, sub_name2, metaname1, subname1 = (
                    match.groups(1),
                    match.groups(2),
                    match.groups(3),
                    match.groups(4),
            ...
            pattern = (
                r"Please search for papers about (.+?) from (\w+) in top (\w+) rank"
            )
    ...
    def pre_rag(self):
        if not os.path.exists(self.data_path):
            if self.raw_datapath is None:
                raise ValueError(
                    "Database is not existing must create it but no raw data"
                )
            else:
            ...
            if re.match(r"Please add", self.task_input):
                ...
                query, metaname, sub_name, method = self.match(
                    self.task_input, "retrieve"
                )
                self.database.retrieve(
                    self.data_path, sub_name, query, method, metaname
                )
            elif re.match(r"Please join", self.task_input):
                metaname2, sub_name2, metaname1, sub_name1 = self.match(
                    self.task_input, "join"
                )
                self.database.join(
                    self.data_path, sub_name1, metaname1, metaname2, sub_name2
                )
                ...
                if "and" in query:
                    ...
                    for i, s in enumerate(slices):
                        que.append(s)
                    ans, name = self.database.from_some_key_full(
                        self.data_path, que, sub_name, con="and"
                    )
                elif "or" in query:
                    que = []
                    ...
                    for i, s in enumerate(slices):
                        que.append(s)
                    ans, name = self.database.from_some_key_full(
                        self.data_path, que, sub_name, con="or"
                    )
                else:
                    ans, name = self.database.from_some_key_full(
                        self.data_path, query, sub_name
                    )
                return ans, name
            elif re.match(r"Please search for papers about", self.task_input):
                top, query, sub_name = self.match(self.task_input, "about")
                ans, name = self.database.from_some_key_sy(
                    self.data_path, query, top, sub_name
                )
                return ans, name
            else:
    ...
    def run(self):
        ...
        for i, step in enumerate(workflow):
            ...
            count = 7
            with open(
                "/Users/manchester/Documents/rag/AIOS/test/file10.txt", "r"
            ) as file:
                for line in file:
                    count += 1
                    start = time.time()
